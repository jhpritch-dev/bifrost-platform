...
graph TB
    subgraph BIFROST_NET["Local LAN · 2.5GbE + Tailscale Overlay"]
        ...
        subgraph HEARTH["HEARTH — Always-On Server (24/7 · UPS)"]
            direction TB
            H_CPU["Ryzen 7 5800G · 64GB DDR4"]
            H_IGPU["Vega 8 iGPU · Vulkan\nEmbedding · Doc Classification"]
        ...
        subgraph FORGE["FORGE — Portable AI Workstation"]
            direction TB
            F_GPU["Radeon 8060S · 96GB Unified VRAM\n128GB LPDDR5X"]
            F_NPU["XDNA 2 NPU · 55 TOPS\nComplexity Classification\nReal-time Embedding"]
            F_CPU["Ryzen AI Max+ 395"]
            F_OLLAMA["Ollama · Tiers 1b/2/2.5\nqwen2.5-coder:14b · qwen2.5:72b Q4"]
            F_LEM["Lemonade Server :8000\nQwen3-0.6b / 1.7b FLM · XDNA 2"]
        end
    end
    ...
    style FORGE fill:#1a1a2e,stroke:#16213e,color:#eee
...
Hearth	—	Vega 8 iGPU (Vulkan)	Embedding + document classification, 24/7
...
2.5	qwen2.5:72b Q4	Forge	~4-8 tok/s	Architecture, deep reasoning	~10%
3-Claude	claude-sonnet-4-6	Cloud	—	Frontier reasoning, code review	~8%
3-Gemini	Gemini 2.0 Flash	Cloud	—	Massive context, doc ingestion	~4%
3-Fast	Groq	Cloud	—	Fast lookups, API docs	~3%
E-base	nomic-embed-text	Hearth Vega 8	—	Codebase embeddings (24/7)	always-on
T-NPU	Qwen3-0.6b / 1.7b FLM	Forge XDNA 2	—	Complexity classification	always-on
E-local	nomic-embed-text	Forge NPU	—	Real-time AUTOPILOT embedding	when online
---
Current Status
Phases 0 + 1 + 2 + 3a + 3b + 3c — COMPLETE. JARVIS validated.
What's Running
[x] Hearth k3d cluster — Observer, Broadcaster, Prometheus, Grafana, ChromaDB operational
[x] Bifrost Ollama — Vulkan backend, qwen2.5-coder:7b + qwen3:14b active
[x] Forge Ollama — qwen2.5-coder:14b + qwen2.5:72b Q4 active
[x] Forge XDNA 2 NPU — Lemonade Server v9.1.4, Qwen3 FLM models, T-NPU tier validated (13s warm inference)
[x] Complexity Router — four-band classifier (TRIVIAL/MODERATE/COMPLEX/FRONTIER) operational
[x] Cascade routing — all bands validated end-to-end
[x] Cloud APIs — Claude (claude-sonnet-4-6), Gemini 2.0 Flash, Groq wired and tested
[x] JARVIS mode — full fleet, all tiers active, zero-cloud-egress validated
[x] AUTOPILOT pipeline — LangGraph DAG, parallel fan-out, 4-level verification harness operational
[x] Semantic verification (Level 4) — nomic-embed-text cosine similarity gate live
[x] Context distillation — Vega 8 Ollama (:11436) running qwen3.5:4b as subtask distiller
[x] PR Review Pipeline — dual-model parallel review, GitHub webhook + comment posting validated
[x] BIFROST Portal Phase 1 — Observe (live fleet), Converse (streaming + RAG + routing pills) live
[x] Static analysis post-processing — ruff on INTERACTIVE output, re-prompt on failure
[x] Prometheus metrics — AUTOPILOT traffic, mode gauge, per-tier routing stats
In Progress
[ ] Portal Command surface (mode switcher, AUTOPILOT launcher)
[ ] MCP tool suite (RAG, filesystem, shell, git servers)
[ ] Telemetry persistence (SQLite `InferenceEvent` schema)
[ ] Web enrichment pipeline (SearXNG + Router intent detection)
[ ] Demo production (6-demo plan)
On the Horizon
[ ] Anthropic-format adapter — Claude Code routing through BIFROST Router locally
[ ] Forge Linux migration — Ubuntu 24.04, ROCm native on gfx1151, Qwen3 family unlocked
[ ] General / research routing profile
[ ] RFP Engine — privacy-first autonomous proposal generation
[ ] Ops Agent — three-layer perception/reasoning/action, k3d deployment
---
Tech Stack
Layer	Technology
Inference Engine	Ollama (Vulkan backend) · Lemonade Server (XDNA 2 NPU)
Orchestration	LangGraph · FastAPI · Pydantic v2 · Instructor
Local Models	Qwen 2.5 Coder (7B, 14B, 32B, 72B) · Qwen3 FLM (NPU)
Cloud APIs	Anthropic Claude · Google Gemini 2.0 Flash · Groq
Cluster	k3d (k3s in Docker) · FluxCD GitOps
Vector Store	ChromaDB · nomic-embed-text
MCP Tool Servers	FastMCP (HTTP transport)
Monitoring	Prometheus · Grafana
Portal	React · Nordic Mission Control dark theme
IDE Integration	VS Code + Continue.dev · Claude Code
Network	2.5GbE LAN · Tailscale overlay
Storage	Tiered HOT/WARM/COLD · SMB shares · ImDisk RAM disk
---
Design Principles
Local-first, privacy-first, on-site-first — Every piece of silicon maximized for its capabilities. Cloud is the last resort.
Zero contention by architecture — Big GPUs generate tokens. Small accelerators classify and embed. CPUs orchestrate. No scheduling conflicts.
Failed attempts are signal, not waste — Each tier's dead ends narrow the solution space for the next tier.
Hardware-topology-aware routing — The system continuously adapts its capabilities based on what hardware is actually online.
Objective verification, not model self-assessment — Six-level harness with embedding similarity checks on dedicated silicon. The model's opinion of its own output is irrelevant.
Hard budgets, no exceptions — Loop limits, spend caps, timeouts, checksums. A model never decides its own loop budget.
---
License
Apache 2.0 (open-core — enterprise licensing path planned)
---
