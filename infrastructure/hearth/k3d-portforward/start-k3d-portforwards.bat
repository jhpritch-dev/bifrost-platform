...
start "k3d-pf-observer" /B kubectl port-forward svc/observer -n inference-platform 8081:8081
echo [%date% %time%] Started: Observer 8081:8081 >> %LOGFILE%

start "k3d-pf-broadcaster" /B kubectl port-forward svc/broadcaster -n inference-platform 8092:8090
echo [%date% %time%] Started: Broadcaster 8092:8090 >> %LOGFILE%

start "k3d-pf-embeddings" /B kubectl port-forward svc/ollama-embed -n context 11435:11434
